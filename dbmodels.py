...
class SensorType(db.Model):
    ...
    name = db.Column(db.String(100), nullable=False)  # Typ czujnika, np. 'light_sensor'
...
class MeasurementType(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)  # Typ pomiaru, np. 'Air Temperature'
...
class Sensor(db.Model):
    ...
    type_id = db.Column(db.Integer, db.ForeignKey('sensor_types.id'), nullable=False)  # Klucz obcy do sensor_types
...
class SensorReading(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    sensor_id = db.Column(db.Integer, db.ForeignKey('sensors.id'), nullable=False)  # Klucz obcy do sensors
    measurement_type_id = db.Column(db.Integer, db.ForeignKey('measurement_types.id'), nullable=False)  # Klucz obcy do measurement_types
    value = db.Column(db.Float, nullable=False)  # Wartość pomiaru
    timestamp = db.Column(db.DateTime, default=lambda: datetime.now().replace(microsecond=0))  # Czas odczytu
